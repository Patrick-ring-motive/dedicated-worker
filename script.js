...
void async function DedicatedWindow() {
  ...
  const myWorker = new Worker(URL.createObjectURL(new Blob(['(' + DedicatedWorker + ')();'], {
    type: "text/javascript"
  })));
  myWorker.ready = new Promise((resolve) => {
    myWorker.readyResolve = resolve
  });
  ...
  globalThis.spellFix = async function() {
    ...
    while (n = walk.nextNode()) {
      ...
      if (!String(n.textContent).trim().length) {
        continue;
      }
      await sleep(20);
...
async function DedicatedWorker() {
  ...
  functions = {
    ...
    suggest: function(x) {
      ...
      if ((!functions.check(y[0])) || /[0-9]/.test(y[0])) {
        y = [];
      }
